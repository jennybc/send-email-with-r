...
```{r include = FALSE}
fake_emails <- emails[1:3] %>% 
  map(function(x) {
    x$header$To <- x$header$Bcc <- 'jenny@stat.ubc.ca'
    x$header$From <- 'stat545ubc@gmail.com'
    x
})
...
#sent_mail <- plyr::llply(fake_emails, send_message, .progress = 'text')
```
